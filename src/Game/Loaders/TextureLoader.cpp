...
bool IsCCW(const Vec2<long double>& a, const Vec2<long double>& b, const Vec2<long double>& c) {
...
// CCW triangles.
bool PointInCircle(const Vec2<long double>& triA, const Vec2<long double>& triB, const Vec2<long double>& triC, const Vec2<long double> point) {
	// translate our triangle poionts, so testpoint is at the origin
	Vec2<long double> newA = triA - point;
	Vec2<long double> newB = triB - point;
	Vec2<long double> newC = triC - point;

	// Oriented area (2x) of sub-triangles)
	long double areaAB = newA.CrossProduct(newB);
	long double areaBC = newB.CrossProduct(newC);
	long double areaCA = newC.CrossProduct(newA);

	long double determinant = (newA.GetMagnitudeSquared() * areaBC) + (newB.GetMagnitudeSquared() * areaCA) + (newC.GetMagnitudeSquared() * areaAB);
...
int FindTriangleContainingPoint(std::vector<Tri>& tris, std::vector<Vec2<long long>>& allPoints, int pointIdx, int startTri) {
	int curr = startTri;
	...
	while (true) {
		...
		for (int e = 0; e < 3; ++e) { // edge
			...
			long double ori = EdgeOrientation({ static_cast<long double>(allPoints[v0].x), static_cast<long double>(allPoints[v0].y) },
				{ static_cast<long double>(allPoints[v1].x), static_cast<long double>(allPoints[v1].y) }, 
				{ static_cast<long double>(allPoints[pointIdx].x), static_cast<long double>(allPoints[pointIdx].y) });
...
int FindNextTriangleMove(const std::vector<Vec2<long long>>& allPoints, const Tri& triangle, const Vec2<long double>& point) {
...
void TriangulateInTriangle(std::stack<int>& dirtyTriangles, std::vector<Tri>& triangles, Tri& parent, int parentIdx, uint64_t pointIdx, std::vector<Vec2<long long>>& allPoints) {
	...
	if (!IsCCW({ static_cast<long double>(allPoints[parent.vIndices[0]].x), static_cast<long double>(allPoints[parent.vIndices[0]].y) },
		{ static_cast<long double>(allPoints[parent.vIndices[1]].x), static_cast<long double>(allPoints[parent.vIndices[1]].y) }, 
		{ static_cast<long double>(allPoints[parent.vIndices[2]].x),  static_cast<long double>(allPoints[parent.vIndices[2]].y) })) {
		throw std::runtime_error("We didn't maintain CCW triangles.");
	}

	if (!IsCCW({ static_cast<long double>(allPoints[t1.vIndices[0]].x), static_cast<long double>(allPoints[t1.vIndices[0]].y) },
		{ static_cast<long double>(allPoints[t1.vIndices[1]].x), static_cast<long double>(allPoints[t1.vIndices[1]].y) },
		{ static_cast<long double>(allPoints[t1.vIndices[2]].x),  static_cast<long double>(allPoints[t1.vIndices[2]].y) })) {
		throw std::runtime_error("We didn't maintain CCW triangles.");
	}

	if (!IsCCW({ static_cast<long double>(allPoints[t2.vIndices[0]].x), static_cast<long double>(allPoints[t2.vIndices[0]].y) },
		{ static_cast<long double>(allPoints[t2.vIndices[1]].x), static_cast<long double>(allPoints[t2.vIndices[1]].y) },
		{ static_cast<long double>(allPoints[t2.vIndices[2]].x),  static_cast<long double>(allPoints[t2.vIndices[2]].y) })) {
		throw std::runtime_error("We didn't maintain CCW triangles.");
	}
...
void TriangulateOnEdge(std::stack<int>& dirtyTriangles, std::vector<Tri>& triangles, int parentIdx, int edgeIdx, uint64_t pointIdx, std::vector<Vec2<long long>>& allPoints) {
	...
	if (!IsCCW({ static_cast<long double>(allPoints[neighbour.vIndices[0]].x), static_cast<long double>(allPoints[neighbour.vIndices[0]].y) },
		{ static_cast<long double>(allPoints[neighbour.vIndices[1]].x), static_cast<long double>(allPoints[neighbour.vIndices[1]].y) },
		{ static_cast<long double>(allPoints[neighbour.vIndices[2]].x),  static_cast<long double>(allPoints[neighbour.vIndices[2]].y) })) {
		throw std::runtime_error("We didn't maintain CCW triangles.");
	}
...
std::vector<TextureLoader::UVTri> TriangulatePolygons(std::vector<Poly> polygons, Texture& t) {
	...
	*/
	...
	std::vector<Vec2<long long>> allPoints;
	...
	for (Poly& p : polygons) {
		...
		for (int i = 0; i < p.outerVertices.size(); ++i) {
			...
			if (vertexIdx.find(vKey) == vertexIdx.end()) {
				...
				allPoints.push_back(Vec2<long long>(v->x, v->y));
	...
	if (vertexIdx.find(MakeEdgeKey(0, t.height)) == vertexIdx.end()) {
		...
		allPoints.push_back({ 0, t.height});
	...
	if (vertexIdx.find(MakeEdgeKey(t.width, t.height)) == vertexIdx.end()) {
		...
		allPoints.push_back({t.width, t.height });
	...
	*/
	...
	if (IsCCW({ static_cast<long double>(-t.width), static_cast<long double>(-t.height)}, { static_cast<long double>(3 * t.width), static_cast<long double>(-t.height)},
		{ static_cast<long double>(-t.width), static_cast<long double>((3 * t.height) + 1)})) {
		allPoints.push_back({ -t.width, -t.height }); // Super Triangle Vert 0
		allPoints.push_back({ 3 * t.width, -t.height }); // Super Triangle Vert 1
		allPoints.push_back({ -t.width, (3 * t.height) + 1 }); // Super Triangle Vert 2
	}
	else {
		allPoints.push_back({ -t.width, -t.height }); // Super Triangle Vert 0
		allPoints.push_back({ -t.width, (3 * t.height) + 1 }); // Super Triangle Vert 1
		allPoints.push_back({ 3 * t.width, -t.height }); // Super Triangle Vert 2
	}
	...
	for (auto& po : allPointsOriginal) {
		...
		while (!dirtyTriangles.empty()) {
			...
			if (!PointInCircle({ static_cast<long double>(allPoints[pNew].x), static_cast<long double>(allPoints[pNew].y) }, 
				{ static_cast<long double>(allPoints[pLeft].x), static_cast<long double>(allPoints[pLeft].y) },
				{ static_cast<long double>(allPoints[pRight].x), static_cast<long double>(allPoints[pRight].y) }, 
				{ static_cast<long double>(allPoints[pOpp].x), static_cast<long double>(allPoints[pOpp].y) })) {
				continue;
			}
			...
			if (!IsCCW({ static_cast<long double>(allPoints[dirtyTri.vIndices[0]].x), static_cast<long double>(allPoints[dirtyTri.vIndices[0]].y) },
				{ static_cast<long double>(allPoints[dirtyTri.vIndices[1]].x), static_cast<long double>(allPoints[dirtyTri.vIndices[1]].y) },
				{ static_cast<long double>(allPoints[dirtyTri.vIndices[2]].x), static_cast<long double>(allPoints[dirtyTri.vIndices[2]].y) })) {
				throw std::runtime_error("We didn't maintain CCW triangles.");
			}

			if (!IsCCW({ static_cast<long double>(allPoints[adjTri.vIndices[0]].x), static_cast<long double>(allPoints[adjTri.vIndices[0]].y) },
				{ static_cast<long double>(allPoints[adjTri.vIndices[1]].x), static_cast<long double>(allPoints[adjTri.vIndices[1]].y) },
				{ static_cast<long double>(allPoints[adjTri.vIndices[2]].x), static_cast<long double>(allPoints[adjTri.vIndices[2]].y) })) {
				throw std::runtime_error("We didn't maintain CCW triangles.");
			}
	...
	*/
	...
	std::vector<TextureLoader::UVTri> uvTriangles;

	for (Tri& tri : triangles) {
		int i0 = tri.vIndices[0];
		int i1 = tri.vIndices[1];
		int i2 = tri.vIndices[2];
		if (i0 == super0 || i0 == super1 || i0 == super2 || i1 == super0 || i1 == super1 || i1 == super2 || i2 == super0 || i2 == super1 || i2 == super2) {
			continue;
		}
...
/*
std::vector<UVTri> ObtainUVTriangles(std::vector<Tri>& triangles, Texture& t) {
	std::vector<UVTri> uvTriangles;

	for (Tri& tri : triangles) {
		uvTriangles.push_back(tri)
	}
}*/


std::vector<TextureLoader::UVTri> TextureLoader::GenerateTextureTopology(std::string& texturePath) {
	// First read in the general TGA texture to get the pixel data
	Texture t = TextureLoader::ProcessTGA(texturePath);
	...
	OutputDebugString(std::to_string(vertsTotal).c_str());
	OutputDebugString("\n");
	std::vector<UVTri> triangles = TriangulatePolygons(polygons, quantizedT);
	OutputDebugString("Triangles: ");
	OutputDebugString(std::to_string(triangles.size()).c_str());
	OutputDebugString("\n");
	int abc = 123;
	//std::vector<UVTri> uvTriangles = ObtainUVTriangles(triangles, quantizedT);
