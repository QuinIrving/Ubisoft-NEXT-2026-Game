...
class ViewVertex {
public:
	ViewVertex(Vec3<float> worldPos, Vec3<float> viewPos, Vec3<float> worldNormal, Vec3<float> viewNormal, Vec3<float> viewTangent, Vec2<float> uv, Vec4<float> colour)
		: m_worldPosition(worldPos), m_viewPosition(viewPos), m_worldNormal(worldNormal), m_viewNormal(viewNormal), m_viewTangent(viewTangent), m_UV(uv), m_colour(colour) {};
